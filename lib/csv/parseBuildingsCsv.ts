 ...
 * code, name, and optionally address (order flexible, case-insensitive).
 */
...
export function parseBuildingsCsv(text: string): ParseBuildingsCsvResult {
  ...
  if (codeIdx < 0 || nameIdx < 0) {
    issues.push({
      ...
      message: 'First row must be a header with "code" and "name" columns. Optional: "address".',
    });
    return { rows: [], issues };
  }
